...
int main(){
    ...
    generate_aco_pheromone_parameters_array_to_superstruct_edges_type(s, 10.0);// generate the array for the pheromone for each particle
    ...
    init_aco_superstruct_general_stuff(s,26,250,10,128,0,10,0.95,0.9,0.5,8,0.7);// init general stuff for aco algorithm
    ...
    for(i = 0; i < s->number_of_iterations; i++){
        ...
        for(j = 0; j < s->n_ants; j++){
            ...
            aco_superstruct_select_params_according_to_pheromone_exploration_levy_flight_edges_type(s);
            for(z = 0; z < threads; z++){
        ...
        update_pheromone_according_to_local_best_edges_type(s);
        ...
        if((count_update_x)%s->time_to_update_with_global_best == 0){
            ...
            update_pheromone_according_to_global_best_edges_type(s);
        }
        ...
        if((s->iteration_index)%s->number_iterations_for_single_aco_iteration == 0){
            printf("X update\n");
            aco_superstruct_update_params_eiwoa_edges_type(s);
        ...
        aco_superstruct_select_params_according_to_best_pheromone_edges_type(s);
