...
int main(){
    ...
    for(i = 0; i < 28; i++){
        ...
        activations[i] = init_aco_activation(GELU,N_ACTIVATION_FUNCTIONS_1_0,fcl_output1_size,fcl_output1_size,i*3+2,0,0,0,NULL,NULL,NULL,generate_all_activations());
        fcls[i]->k_percentage = 1;
        if(i != 27)
        lstms[i]->k_percentage = 0.5;
    }
    
    
    
    aco_fcl* node2 = init_aco_fcl(fcl_input2_size,fcl_output2_size,84,0,0,0,0,0,0,NULL,NULL,NULL,NULL,NULL,NULL);
    aco_activation* node3 = init_aco_activation(GELU,N_ACTIVATION_FUNCTIONS_1_0,fcl_output2_size,fcl_output2_size,85,0,0,0,NULL,NULL,NULL,generate_all_activations());
    aco_fcl* node4 = init_aco_fcl(fcl_input3_size,fcl_output3_size,86,0,0,0,0,0,0,NULL,NULL,NULL,NULL,NULL,NULL);
    aco_activation* node5 = init_aco_activation(SOFTMAX,0,fcl_output3_size,fcl_output3_size,87,0,0,0,NULL,NULL,NULL,NULL);
    node2->k_percentage = 1;
    ...
    edge_popup_trainer* e = init_edge_popup_trainer(ss,threads,0,mini_batch_size,error_flags,0.0001,0.00000001,0.975,0.999);
    ...
    for(i = 0; i < epochs; i++){
        ...
        for(j = 0; j < (instances/divisor_of_instances); j+=mini_batch_size){
            ...
            /*printf("Scores:\n");
            print_vector(e->arrays[0][2],28*100);
            if(e->arrays[0][2][0] != e->arrays[0][2][0])
                exit(0);*/
        }
