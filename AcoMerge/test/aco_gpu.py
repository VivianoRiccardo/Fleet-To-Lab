...
model.initialize_aco_eiwoa(number_of_parameters, initial_pheromone, p=0.8)
modelGpu = pyezsparkc.acoSuperstructGpu(batch_size_gpu, 1, model, '../src/kernels_gpu.cl')
...
l = [1]*batch_size_gpu
modelGpu.set_paths_to_use(l)
print("training")
modelGpu.assign_array(inputs)
input_indices = [0]*batch_size_gpu
for i in range(epochs):
    ...
    for k in range(number_of_parameters):
        loss = 0
        modelGpu.assign_path_to_superstruct_exploration_array(0)
        model.random_select_network()
        modelGpu.put_all_paths_on_gpu()
        network_output_gpu =[]
        for j in range(0,50000,batch_size_gpu):
            for z in range(batch_size_gpu):
                input_indices[z] = z+j
            network_output = modelGpu.forward_local(input_indices, 0)
            loss += cross_entropy(outputs[j:j+batch_size_gpu].flatten(),network_output)
            network_output_gpu = [item for sublist in [network_output_gpu, network_output] for item in sublist]
        '''
        for j in range(int(50000/batch_size_gpu)):
            network_output = modelGpu.forward(inputs[j*batch_size_gpu:(j+1)*batch_size_gpu])
            loss += cross_entropy(outputs[j*batch_size_gpu:(j+1)*batch_size_gpu].flatten(),network_output)
            #network_output_gpu = [item for sublist in [network_output_gpu, network_output] for item in sublist]
        '''
        #network_output = model.forward(inputs)
        #loss2 = cross_entropy(outputs.flatten(),network_output)
        #print('pheromones')
        #print('pheromone cpu: '+str(500000.0/loss2))
        #print('pheromone gpu: '+str(500000.0/loss))
        
        pheromone = 500000.0/loss
        if pheromone > global_best:
            global_best = pheromone
        model.set_local_pheromone(pheromone)
        model.replace_local_best_with_current_for_gpu()
    iteration_index += 1
    count_update_x +=1
